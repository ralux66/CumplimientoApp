    ...
    if (bundle.hasOwnProperty('src') && bundle.hasOwnProperty('bundle')) {
      ...
      if ('mandatory' in bundle.src && 'optional' in bundle.src) {
        ...
        for (var vendor in vendors) {
          ...
          for (var type in vendorObj) {
            ...
            switch (type) {
              case 'fonts':
                stream = gulp.src(vendorObj[type]).pipe(_self.outputChannel(bundle.bundle[type] + '/' + vendor)());
                streams.push(stream);
                break;
              case 'images':
                stream = gulp.src(vendorObj[type]).pipe(_self.outputChannel(bundle.bundle[type] + '/' + vendor)());
      ...
      for (var type in bundle.src) {
        ...
        switch (type) {
          ...
          case 'images':
            if (bundle.bundle.hasOwnProperty(type)) {
              stream = gulp.src(bundle.src[type]).pipe(_self.outputChannel(bundle.bundle[type])());
              streams.push(stream);
            }
            break;
        }
        ...
        switch (type) {
          case 'styles':
            if (bundle.output.hasOwnProperty(type)) {
              stream = gulp.src(bundle.src[type]).pipe(_self.cssChannel()()).pipe(_self.outputChannel(bundle.output[type])());
              streams.push(stream);
            }
            break;
          case 'scripts':
            ...
            break;
          default:
            stream = gulp.src(bundle.src[type]).pipe(_self.outputChannel(bundle.output[type])());
            streams.push(stream);
            break;
        }
      }
